...
int main( int argc, char *argv[ ] ){
        ...
        printf("Average MegaBodies Compared Per Second: %8.2lf\n",sum/NUMSAMPLE);
...

{
         ...
         {
                //******************************************************************************************************
                //  "coarse-grained parallelism" means putting the OpenMP #pragma omp parallel forbefore the i for-loop. 
                ...
                // # pragma omp parallel for default(none),shared(Bodies),schedule(dynamic)
...

                 {
                        ...
                        // Fine - Static
                        // # pragma omp parallel for default(none),reduction(+:fx,fy,fz),shared(Bodies),shared(i,bi)

                        // Fine - Dynamic
                        # pragma omp parallel for default(none),reduction(+:fx,fy,fz),shared(Bodies),schedule(dynamic),shared(i,bi)
        ...
        float res = (NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.;

         // print performance here:::
         // "MegaBodies Compared Per Second" ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.)
        printf("MegaBodies Compared Per Second: %8.2lf\n",res);
